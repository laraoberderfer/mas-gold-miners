    ...
    boolean pick(int ag) {
        ...
        if (hasObject(WorldModel.GOLD, l.x, l.y)) {
            if (!isCarryingGold(ag)) {
                ...
                logger.warning("Agente " + (ag + 1) + " está tentando pegar ouro, mas já está carregando ouro!");
            }
        } else {
            logger.warning("Agente " + (ag + 1) + " está tentando pegar ouro, mas não existe ouro em " + l.x + "x" + l.y + "!");
        }
        return false;
    }
    ...
    boolean drop(int ag) {
        ...
        if (isCarryingGold(ag)) {
            if (l.equals(getDepot())) {
                goldsInDepot++;
                logger.info("Agente " + (ag + 1) + " carregando ouro para o depósito!");
            } else {
